...
designParams.polyVertices2params = polyVertices2params;
while (ischar(line))
    line = strtrim(line);
     if (strncmpi(line,designKeyword,length(designKeyword)))
        splitStr = regexp(line,',','split');
        designNum = str2double(splitStr{2});
        if (isnan(designNum))
            error('design Keyword should be followed by comma and design variable number')
        end
        line = fgetl(fileID);
        splitStr = regexp(line,',','split');
