...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% this function computes the error of the FEM solution 
% in a polynomial IGFEM element 
...
for c=1:numel(parent.child)
    ...
    if (parent.child(c).isTriangle)
        for i = 1:size(gauss.elem,2)
            ...
            Xglo = Xch*Nch;
            [u,u_x,u_y] = refSoln(Xglo(1),Xglo(2));
            UU = UUR(parent.nodes)'*N;
            varargout{1} = varargout{1} + (UU-u)^2*factor;
            varargout{2} = varargout{2} + u^2*factor;
            if (nargout > 2)
                dU = UUR(parent.nodes)'*B;
                varargout{3} = varargout{3} + ((UU - u)^2 ...
                                            + (dU(1) - u_x)^2 ...
                                            + (dU(2) - u_y)^2)*factor;
                varargout{4} = varargout{4} + (u^2 + u_x^2 + u_y^2)*factor;
            end
        end
    else
