...
parentFields2remove = {'collinearLoc'};
...
for p = 1:nElems
    ...
    if(parent(p).type > 1)       
        ...
        constraint = parent(p).channelLocNodes';
        DT = delaunayTriangulation(pts,constraint);
                ...
                for j=i+1:nTri
                    ...
                    %if(isempty(find(delTR==j, 1)))
                        ...
                        if(length(cij)==2)
                            ...
                            for k = 1:nChannels
                                com=setdiff(cij, parent(p).channelLocNodes(:,k));
                                if(isempty(com))
                                    edgeConstraint=true;
                                    break;
                                end
        ...
        % delete triangles which have been merged
        
        TR(delTR,:)=[];
        locNodes = [[TR,zeros(size(TR,1),1)];Rec(1:nRec,:)];
        ...
        parentCopy = child_elements_channels(parent(p),polyIGFEM);
        ...
        % child element
        recalcChildChannels = false;
        
        for c = 1:numel(parentCopy.child)
            % swap the edge of ajacent child elements if one child
            % element has two line sources
            nChannels = numel(parentCopy.child(c).channelNum); 
            if (nChannels == 2)
                ...
                lsEdge1 = parentCopy.child(c).channelLocNodes(1,1)... 
                         +parentCopy.child(c).channelLocNodes(2,1);
                lsEdge2 = parentCopy.child(c).channelLocNodes(1,2)... 
                         +parentCopy.child(c).channelLocNodes(2,2);
                nonLsEdge = setdiff([3,5,4],[lsEdge1,lsEdge2],'stable');
                % nodes of the edge to be swapped
                edgeLocNodes = parentCopy.child(c).locPaNodes(localNodeSets(nonLsEdge-2,:))';
                ...
                % nodes of child elements sharing the edge to be
                % swapped
                if(~isempty(c2))
                    adjChildrenLocNodes = [parentCopy.child(c).locPaNodes(1:3)'; ...
                                            parentCopy.child(c2).locPaNodes(1:3)'];
                    ...
                    recalcChildChannels = true;
                else
                    if (parentCopy.child(c).isTriangle)
...
parent = rmfield(parent,parentFields2remove);
end
