...
function [parent,cstrElems,nIGFEMelems] ...
                = parent_elements_nurbs(parent, ...
                                        ...
                                        polyIGFEM,...
                                        calcVel)
%if (isfield(parent,'refine'))
%    parent = rmfield(parent,'refine');
%end
if (calcVel)
     fields2remove = {'coincident','seg','locEdge','edge','nurbsParam',...
                     'multItrsect','vx','vy','auxDesignParamNum'};
else
    fields2remove = {'coincident','seg','locEdge','edge','nurbsParam',...
                     'multItrsect'};
end
%fields2remove = {'seg','locEdge','edge','nurbsParam','multItrsect'};
...
for i = 1:size(elem_nodes,1) 
    ...
    if(parent(i).nItrsect > 1)
        ...
        if(~isempty(junc) && junc(i).nJunc)
            %nJunc = junc(i).nJunc;
            %hasJunc = true;
            parent(i).nItrsect = parent(i).nItrsect+junc(i).nJunc;
            parent(i).nodes = [parent(i).nodes,junc(i).node];
            parent(i).edge = [parent(i).edge,zeros(1,junc(i).nJunc)];
            parent(i).locEdge = [parent(i).locEdge,zeros(1,junc(i).nJunc)];
            parent(i).nSeg = [parent(i).nSeg,junc(i).nSeg];
            parent(i).seg = [parent(i).seg,junc(i).seg];
            parent(i).nurbsParam = [parent(i).nurbsParam, ...
                                         junc(i).nurbsParam];
                                     
            % calculate velocity here!  
            if (calcVel)
                kold = numel(parent(i).vx);
                parent(i).vx = [parent(i).vx,cell(1,junc(i).nJunc)];
                parent(i).vy = [parent(i).vy,cell(1,junc(i).nJunc)];
                parent(i).auxDesignParamNum = [parent(i).auxDesignParamNum,cell(1,junc(i).nJunc)];
                for k = 1:junc(i).nJunc
                    m = kold + k;
                    parent(i).vx{m} = [1,0];
                    parent(i).vy{m} = [0,1];
                    % multiple channels connected to a branching point
                    % so take one channel. in this case, it is
                    % junc(i).seg{k}(1). find the control point number and
                    % then the design parameter number
                    chanNum = junc(i).seg{k}(1);
                    if(channels.contvty(chanNum,1) == junc(i).ipt(k))
                        ctrlPtNum = 1;
                    elseif(channels.contvty(chanNum,2) == junc(i).ipt(k))
                        ctrlPtNum = channels.nurbs(chanNum).number;
                    else
                        error('cannot find channel with branching point %i', junc(i).ipt(k))
                    end
                    parent(i).auxDesignParamNum{m} ...
                        = channels.designParamNum{chanNum}(1:2,ctrlPtNum)';
                end
            end
        end


        if(~isempty(kinks) && kinks(i).nKinks)
            %nJunc = junc(i).nJunc;
            %hasJunc = true; % elements with branching points and kinks are treated similarly
            parent(i).nItrsect = parent(i).nItrsect+kinks(i).nKinks;
            parent(i).nodes = [parent(i).nodes,kinks(i).node];
            parent(i).edge = [parent(i).edge,zeros(1,kinks(i).nKinks)];
            parent(i).locEdge = [parent(i).locEdge,zeros(1,kinks(i).nKinks)];
            parent(i).nSeg = [parent(i).nSeg,kinks(i).nSeg];
            parent(i).seg = [parent(i).seg,kinks(i).seg];
            parent(i).nurbsParam = [parent(i).nurbsParam, ...
                                    kinks(i).nurbsParam];
            ...
            if (calcVel)          
                kold = numel(parent(i).vx);
                parent(i).vx = [parent(i).vx,cell(1,kinks(i).nKinks)];
                parent(i).vy = [parent(i).vy,cell(1,kinks(i).nKinks)];
                parent(i).auxDesignParamNum = [parent(i).auxDesignParamNum,cell(1,kinks(i).nKinks)];
                for k = 1:kinks(i).nKinks
                    m = kold + k;
                    parent(i).vx{m} = [1,0];
                    parent(i).vy{m} = [0,1];
                    parent(i).auxDesignParamNum{m} ...
                        = channels.designParamNum{kinks(i).seg{k}}(1:2,kinks(i).ctrlPtNum(k))';
                end
            end
        end
