...
%              the edge
%   elem_edge: size 2x3 array of the connectivity of the 2 elements sharing
%              the edge
% OUTPUT:
...
function [flip_edge_node,elem_node,elem_edge] = flip_edge(edge_node,elem_node,elem_edge)
      ...
      sharedLocNode1 = find(elem_node(1,:)==edge_node(flip_edge,1)); % it is also possible to do find(elem_node(1,:)==edge_node(2))
      sharedLocNode2 = find(elem_node(2,:)==edge_node(flip_edge,2));
