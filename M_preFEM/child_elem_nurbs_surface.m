...
%   channelLocNodes: a length 2 (1 nurbs curve) or 4 (2 nurbs curves) of the 
...
% OUTPUT:
%   nurbsSurf
function [nurbsSurf,uv,uvParam]=child_elem_nurbs_surface(xel,channelLocNodes,nurbsSeg)
if(size(xel,2)==3)
    shape=1;
elseif(size(xel,2)==4)
    shape=2;
...
if(shape==1)
    if(nNurbsSeg == 1)
        otherLocNode=setdiff(1:3,channelLocNodes(1:2,1));
        if(isCCW(channelLocNodes(1:2,1),3))
    ...
    elseif(nNurbsSeg == 2)
        ...
        if(isCCW(channelLocNodes(1:2,2),3))    
...
elseif(shape == 2)
    if(nNurbsSeg == 1)
        if(isCCW(channelLocNodes(1:2,1),4))
            surfPt(:,:,1) = nurbsSeg(1).coefs;
        else
            surfPt(:,:,1) = nurbsSeg(1).coefs(:,end:-1:1);
        end
        nonSourceLocNode = setdiff(1:4,channelLocNodes(1:2,1));
    ...
    elseif(nNurbsSeg == 2)
        ...
        commonLoc = intersect(channelLocNodes(1:2,1),channelLocNodes(1:2,2),'stable');
        if(isempty(commonLoc))
            ...
            if(isCCW(channelLocNodes(1:2,2),4))
                surfPt(:,:,2) = nurbsSeg(2).coefs(:,end:-1:1);            
            else
                surfPt(:,:,2) = nurbsSeg(2).coefs;
            end        
        ...
        % the two nurbs segments intersect at one of their end points    
            ...
            otherNode = setdiff(1:4,channelLocNodes(:));
            isCCW1 = isCCW(channelLocNodes(1:2,1),4);
            isCCW2 = isCCW(channelLocNodes(1:2,2),4);
            if(~isCCW1)
                channelLocNodes(1:2,1)=channelLocNodes(2:-1:1,1);
            end
            if(~isCCW2)     
                channelLocNodes(1:2,2)=channelLocNodes(2:-1:1,2);  
            end        
            
            commonLocOnSeg1 = find(channelLocNodes(1:2,1)==commonLoc,1,'first'); 
            if(commonLocOnSeg1 == 1) 
                % swap line sources
                [channelLocNodes(1:2,2),channelLocNodes(1:2,1)] ...
                        = deal(channelLocNodes(1:2,1),channelLocNodes(1:2,2)); 
            ...
            sLocNode=channelLocNodes(1,1);
            eLocNode=channelLocNodes(2,2);
