...
function [KEL,PEL, errflag]   =   compute_regular_element(nodeCoords,...
                          elem_node,parent,elemHeatSource,conductivity,...
                          heatSourceFunc,convect,itrface,gauss)
...
for j = 1:gauss.tri2D.npt
    ...
    KEL  = KEL + (B*Cmat*B'+convect.coef*(N*N'))*detJ*w;
    PEL  = PEL + N*elemHeatSource*detJ*w;
    XX=Xel*N;
    if(~isempty(heatSourceFunc))
        PEL=PEL+N*heatSourceFunc(XX(1),XX(2))*detJ*w;
    end
end
...
if(parent.hasLineSource)
    if(itrface.modelType == 2)
        ...
        for i=1:parent.nLineSource
            ...
            lineSourceVec = nodeCoords(parent.lineSource(i,2),:)-nodeCoords(parent.lineSource(i,1),:);
            ...
            for j = 1:gauss.tri1D.npt
                ...
                % since two elements share a line source at the edge, the
                % contribution to KEL is halved.
                KEL = KEL+0.5*itrface.mcf(parent.lineSourceSeg(i))*N...
                    *lineSourceVec*B'*gauss.tri1D.weight(j);
            end
        end
    end
end
