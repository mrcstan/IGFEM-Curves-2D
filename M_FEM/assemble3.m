...
%
KPP = zeros(mesh.node.Dirichlet.n_pre_temp, mesh.node.Dirichlet.n_pre_temp);
KPF = zeros(mesh.node.Dirichlet.n_pre_temp, n_dof);
KFP = zeros(n_dof, mesh.node.Dirichlet.n_pre_temp);
...
for elem_num = 1:mesh.elem.n_elem 
    ...
    if (mesh.elem.parent(elem_num).type > 1)
        %{
        if(any(elemSet == elem_num))
            ...
            disp(elem_num)
             [KEL,PEL]=compute_parent_element_nurbs(mesh.node.coords,...
                ...
                channels,mesh.heatSource,gauss2);
        else
        %}   
        [KEL,PEL,errflag] ...
        ...
        current_elem_nodes = mesh.elem.parent(elem_num).nodes; 
    else
        [KEL,PEL,errflag] ...
         ...
         current_elem_nodes = mesh.elem.elem_node(elem_num,:);
    end
                          
    [KPP,KPF,KFP,KFF,PP,PF] = element_stiffness_assemble(current_elem_nodes, ...
                                                         eq_num,KEL, PEL, ... 
