...
for c=1:parent.nChild
    ...
    if (supg && parent.child(c).nLineSource)
        unitTan = nodeCoords(parent.child(c).node(parent.child(c).lineSourceLoc(:,2)),:)...
                  -nodeCoords(parent.child(c).node(parent.child(c).lineSourceLoc(:,1)),:);      
        % strategies 2 and 3    
        %{
        % unitTan = bsxfun(@rdivide,unitTan,sqrt(sum(unitTan.^2,2))); % stragegy 3
        if (parent.nLineSource > 1)
            unitTan = sum(bsxfun(@times,unitTan,itrface.mcf(parent.child(c).seg)),1);
        end
        %}
                ...
                for k = 1:gauss.qua1Dt.npt
                    for m = 1:gauss.qua1Dn.npt
                        ...
                        if (supg)
                            % strategy 1
                            %
                            for n = 1:parent.child(c).nLineSource
            ...
            if(parent.child(c).nurbsSeg(i).knots(j)~=parent.child(c).nurbsSeg(i).knots(j+1))
                ...
                for k=1:gauss.qua1Dt.npt
                    ...
                    if(itrface.modelType == 2)
                        ...
                        PEL = PEL + 0.5*conductVal*itrface.Tin(parent.child(c).seg(i))*Wfunc*factor;
                    else
                          KEL=KEL+0.5*itrface.mcf(parent.child(c).seg(i))*Wfunc*(Bp2p*dxiParam(1:2))'*Jp2n*w; 
                    end
