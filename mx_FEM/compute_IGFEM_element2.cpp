...
OUTPUT:
    ...
    errFlag = 0: no negative or zero Jacobian
            < 0: -errFlag is the largest child element number with neg or zero Jacobian
        ...
        if (supg && parent1.children(ch).channelNum.n_elem > 0)
            if (parent1.children(ch).shape == TRIANGLE)
            {
                for (std::size_t i = 0; i < gauss1.elem.n_cols; i++)
                {
                    ...
                    /*
                    Wfunc = N;                   
                    for (std::size_t j = 0; j < parent1.children(ch).channelNum.n_elem; j++)
                    {
                        streamwise_elem_length(he,Bsw,unitChannelVecs.col(j),B);
                        Wfunc = Wfunc + he*Bsw;
                    }
                    */
