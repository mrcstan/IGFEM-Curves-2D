              ...
              bool supg)
{
    ...
    #pragma omp parallel shared(nodeCoords,elemNodes,elemHeatSource, \
                                eqNum, gauss1, parents, channels, supg, \
                                ii,jj,Kval,Pglo,locVecSize,locCumSum, \
                                estSize, paLocOrigNodes) 
    {
        ...
        */
        ...
        for(int i = istart; i < iend; ++i)   
        {
            ...
            else if (parents(i).type == IGFEM)
            {
                ...
                */
                ...
                if (errFlag < 0)
                {
                    omp_set_lock(&writelock);
                    {
                        std::cerr << "warning: compute_IGFEM_element: thread " << threadNum
                        << ", negative Jacobian in parent " << i 
                        << ", child " << -errFlag << std::endl;
                    }
                    omp_unset_lock(&writelock);
                }
                /*
            ...
            {
                std::cerr << "warning: assemble: growing locii" << std::endl;
