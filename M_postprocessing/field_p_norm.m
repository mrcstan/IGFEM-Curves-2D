...
function val = field_p_norm(elem,nodeCoords,UUR,pow,normalization,gauss)
    ...
    for el = 1:size(elem.elem_node,1)
        if (elem.parent(el).type > 1)
            for c = 1:numel(elem.parent(el).child)
                ...
                val = val+mean(UUR(elem.parent(el).child(c).nodes).^pow)*childArea;
            end
        else
            elemArea = abs(polygonArea(nodeCoords(elem.elem_node(el,:),:)));
            %totArea = totArea + elemArea;
            val = val+mean(UUR(elem.elem_node(el,:)).^pow)*elemArea;
        end
    end
else
    iPON = 1:3;
    for el = 1:size(elem.elem_node,1)
        ...
        if (elem.parent(el).type == 2)
            ...
            for c = 1:numel(elem.parent(el).child)
                ...
                for j = 1:size(gauss.elem,2)
                      ...
                      val = val ...
                          + (UUR(elem.parent(el).nodes)'*Np2p)^pow ...
        ...
        elseif (elem.parent(el).type == 3)
            ...
            for c = 1:numel(elem.parent(el).child)
                ...
                val = val + (UUR(elem.parent(el).nodes)'*Np2p).^pow*factor;      
            ...
            for j = 1:size(gauss.elem,2)
                ...
                val = val + (UUR(elem.elem_node(el,:))'*N)^pow ...
                            *gauss.elem(end,j)*detJ;
            end
        end
    end
